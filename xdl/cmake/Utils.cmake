...
# Usage:
#   xdl_list_unique(<list_variable> [<list_variable>] [...])
macro(xdl_list_unique)
  foreach(__lst ${ARGN})
    if(${__lst})
...
# Usage:
#   xdl_clear_vars(<variables_list>)
macro(xdl_clear_vars)
  foreach(_var ${ARGN})
    unset(${_var})
  endforeach()
endmacro()
...
# Usage:
#   xdl_string_unique(<string_variable>)
function(xdl_string_unique __string)
  if(${__string})
    set(__list ${${__string}})
...
# Usage:
#   xdl_merge_flag_lists(out_variable <list1> [<list2>] [<list3>] ...)
function(xdl_merge_flag_lists out_var)
  set(__result "")
  foreach(__list ${ARGN})
...
# Usage:
#   xdl_convert_absolute_paths(<list_variable>)
function(xdl_convert_absolute_paths variable)
  set(__dlist "")
...
# Usage:
#   xdl_parse_header(<file> <define1> <define2> <define3> ..)
macro(xdl_parse_header FILENAME FILE_VAR)
  set(vars_regex "")
...
# Usage:
#   xdl_parse_header_single_define(<library_name> <file> <define_name>)
function(xdl_parse_header_single_define LIBNAME HDR_PATH VARNAME)
  set(${LIBNAME}_H "")
  if(EXISTS "${HDR_PATH}")
...
# Usage:
#   xdl_option(<option_variable> "doc string" <initial value or boolean expression> [IF <condition>])
function(xdl_option variable description value)
  set(__value ${value})
  set(__condition "")
...
# Usage:
#   xdl_compare_lists(<list_variable> <list2_variable> [description])
function(xdl_compare_lists list1 list2 desc)
  set(__list1 ${${list1}})
  set(__list2 ${${list2}})
  list(SORT __list1)
  list(SORT __list2)
...
# Usage:
#   xdl_warnings_disable(<CMAKE_[C|CXX]_FLAGS[_CONFIGURATION]> -Wshadow /wd4996 ..,)
macro(xdl_warnings_disable)
  set(_flag_vars "")
  set(_msvc_warnings "")
  set(_gxx_warnings "")
  ...
  xdl_clear_vars(_flag_vars _msvc_warnings _gxx_warnings)
endmacro()

################################################################################################
# Helper function get current definitions
# Usage:
#   xdl_get_current_definitions(<definitions_variable>)
function(xdl_get_current_definitions definitions_var)
...
# Usage:
#   xdl_get_current_cflags(<cflagslist_variable>)
function(xdl_get_current_cflags cflags_var)
  get_property(current_includes DIRECTORY PROPERTY INCLUDE_DIRECTORIES)
  xdl_convert_absolute_paths(current_includes)
  xdl_get_current_definitions(cflags)

  foreach(i ${current_includes})
    list(APPEND cflags "-I${i}")
  endforeach()

  xdl_list_unique(cflags)
...
# Usage:
#   xdl_parse_linker_libs(<xdl_LINKER_LIBS_var> <directories_var> <libflags_var> <frameworks_var>)
function(xdl_parse_linker_libs xdl_LINKER_LIBS_variable folders_var flags_var frameworks_var)
  ...
  foreach(list_elem ${${xdl_LINKER_LIBS_variable}})
  ...
  xdl_list_unique(libflags folders)
...
# Usage:
#   xdl_detect_darwin_version(<version_variable>)
function(xdl_detect_darwin_version output_var)
  if(APPLE)
...
function(xdl_source_group group)
...
function(xdl_add_test dir extension)
...
function(xdl_add_bench dir extension)
  file(GLOB TEST_SOURCE "${dir}/*${extension}")
  foreach(file ${TEST_SOURCE})
    string(REGEX REPLACE ".*${dir}/" "" file_exe0 ${file})
    string(REPLACE "${extension}" "" file_exe ${file_exe0})
...
function(xdl_add_cuda_bench dir extension)
  file(GLOB TEST_SOURCE "${dir}/*${extension}")
  foreach(file ${TEST_SOURCE})
    string(REGEX REPLACE ".*${dir}/" "" file_exe0 ${file})
    string(REPLACE "${extension}" "" file_exe ${file_exe0})
