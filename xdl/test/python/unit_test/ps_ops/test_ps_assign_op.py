...
class TestPsAssignOp(unittest.TestCase):
    def test_all(self):
        ...
        op = xdl.ps_assign_op(var_name="w", var_type="index", delta=np.array([1,2,3,4], dtype=np.int32))
        execute(op)
        ret = execute(var.value)
        self.assertTrue((ret == np.array([1,2,3,4])).all())

def suite():
    return unittest.TestLoader().loadTestsFromTestCase(TestPsAssignOp)
