...
TEST(FeatureOpTest, CartesianProduct) {
  bool is_sort = false;
  const int num = 3;
  std::string name_arr[] = {"ad_cate_id", "fake_id", "nick_cate_pv_14"};
  int size_arr[] = {3, 5, 2};
  std::vector<int64_t> key_arr[num] = { std::vector<int64_t>({2, 4, 5}),
                                        std::vector<int64_t>({1, 2, 3, 4, 5}),
                                        std::vector<int64_t>({0, 2}) };
  std::vector<float> value_arr[num] = { std::vector<float>({0.1, 0.2, 0.2}),
  ...
  SchemaAddOption(schema, "z_ad_cate_pv_14_match_sum",
                  "Name=z_ad_cate_pv_14_match_sum; Expr=sum(cartesian(max(ad_cate_id),log(nick_cate_pv_14))); Type=Numeric;");
  // Expr = sum(cartesian(5:0.2 ~ 0:log(0.1), 2:log(0.3))) = 0.2*log(0.1) + 0.2*log(0.3)
  ...
  EXPECT_EQ(feature.values(0).key(), 0);
  float expected_result = 0.2 * LogFeature::TransformValue(0.1) + 0.2 * LogFeature::TransformValue(0.3);
