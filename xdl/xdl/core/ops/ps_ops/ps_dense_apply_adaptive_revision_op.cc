...
class PsDenseApplyAdaptiveRevisionOp : public xdl::OpKernelAsync {
 public:
  ...
  void Compute(OpKernelContext* ctx, Callback done) override {
    ...
    Tensor t_max_revision_ratio;
    XDL_CHECK_STATUS_ASYNC(ctx->GetInput(2, &t_max_revision_ratio), done);
    double max_revision_ratio = t_max_revision_ratio.Scalar<double>();
    Tensor t_worker_cnt;
    XDL_CHECK_STATUS_ASYNC(ctx->GetInput(3, &t_worker_cnt), done);
    int64_t worker_cnt = t_worker_cnt.Scalar<int64_t>();
    Tensor t_worker_idx;
    XDL_CHECK_STATUS_ASYNC(ctx->GetInput(4, &t_worker_idx), done);
...
XDL_DEFINE_OP(PsDenseApplyAdaptiveRevisionOp)
  .Input("learning_rate", DataType::kDouble)
  .Input("initial_accumulator_value", DataType::kDouble)
  .Input("max_revision_ratio", DataType::kDouble)
...
XDL_REGISTER_KERNEL(PsDenseApplyAdaptiveRevisionOp, 
                    PsDenseApplyAdaptiveRevisionOp)
  .Device("CPU");
