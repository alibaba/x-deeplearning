...
class PsRestoreOp : public xdl::OpKernelAsync {
 public:
  ...
  void Compute(OpKernelContext* ctx, Callback done) override {
    ...
    client->Restore(ckpt_version, cb);
  }
};

XDL_DEFINE_OP(PsRestoreOp)
  .Input("ckpt_version", DataType::kInt8);

XDL_REGISTER_KERNEL(PsRestoreOp, PsRestoreOp).Device("CPU");
