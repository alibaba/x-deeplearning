...
future<> output_stream<CharType>::write(net::packet p) {
    ...
    if (p.len() != 0) {
        ...
        if (_zc_bufs.len() >= _size) {
            /*
            // In queue write mode, here will be always false!
            // default value : false
            if (false) {
		...
		return zero_copy_put(std::move(_zc_bufs));
                // you must move _zc_bufs NOW!
                // or it will be override by the later data
...
// read n bytes data into CharType[] array
// the CharType must: sizeof(CharType) == 1
template <typename CharType>
future<size_t>
input_stream<CharType>::read_exactly_into(size_t n, CharType* user_buf) {
    // from temporary_buffer rule
    ...
    return _fd.get(user_buf, n).then([this, user_buf, n] (auto size) mutable {
        ...
        if (size < n) {
            return this->read_exactly_into(n - size, user_buf + size).then([size] (auto new_size) {
...
output_stream<CharType>::poll_flush(bool one_more_flush) {
    ...
    _flush = false;
    // we cant set _flushing = true here cause the write_lock
...
template <typename CharType>
void
output_stream<CharType>::set_batch_flushes(bool batch_flush) {
    _batch_flushes = batch_flush;
}

template <typename CharType>
uint64_t
output_stream<CharType>::get_truely_send_count() {
    return _fd.get_truely_send_count();
}

template <typename CharType>
void
output_stream<CharType>::reset_truely_send_count() {
    _fd.reset_truely_send_count();
}
