...
app_template::run_deprecated(int ac, char ** av, std::function<void ()>&& func) {
    ...
    unsigned queue_id = engine().cpu_id();
    auto exit_code = engine().run();

    int64_t user_thread_id = smp::stop_user_thread_id;
    uint64_t seq = smp::stop_sequence;
    smp::cleanup();

    // Both in normal mode and server_client mode,
    // client should send the stop cmd.
    if (user_thread_id != -1 && seq != 0) {
        ...
        // put the stop result to queue
        ps::network::PsWorkItem* pItem = new ps::network::PsWorkItem();
        pItem->SetSideSequ(seq);
        qhr.second->Enqueue(pItem, queue_id, user_thread_id, NULL);
    }
