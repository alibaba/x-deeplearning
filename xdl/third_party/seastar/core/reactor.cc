...
#include "task.hh"
#include "reactor.hh"
#include "ps_queue_hub/queue_hub.hh"
#include "ps_queue_hub/queue_work_item.hh"
#include "memory.hh"
...
// define min user's singal value
#define SIGUSERMIN SIGRTMIN + 10

// signal for wake up epoll_wait
#define SIGWAKEUPEPOLL SIGUSERMIN + 1
...
reactor::reactor(unsigned id)
    ...
    , _cur_poll_mode_thread_count(0)
    , mWriteQueuePoller(new ps::network::WriteQueuePoller()) {
    //, mQueuePoller(new ps::network::QueuePoller(cpu_id())) {

    // If in seperate server_client mode
    // client cpu id range is [0, clientCoreCount)
    // server cpu is range is [clientCoreCount, smp::Count)
    // here we should switch cpu id to MxN queue index
    if (smp::seperate_server_client) {
        //std::cout << "reactor seperate mode." << std::endl;
        // server engine
        if (cpu_id() >= smp::client_count) {
            std::unique_ptr<ps::network::QueuePoller> tmp(new ps::network::QueuePoller(cpu_id() - smp::client_count, "SEASTAR2"));
            mQueuePoller2 = std::move(tmp);
        } else {
            std::unique_ptr<ps::network::QueuePoller> tmp(new ps::network::QueuePoller(cpu_id()));
            mQueuePoller = std::move(tmp);
        }
    } else {
        //std::cout << "reactor not seperate mode." << std::endl;
        std::unique_ptr<ps::network::QueuePoller> tmp(new ps::network::QueuePoller(cpu_id()));
        mQueuePoller = std::move(tmp);

        if (smp::is_server_client_mode) {
            //std::cout << "set is_server_client_mode true, start queue SEASTAR2." << std::endl;
            std::unique_ptr<ps::network::QueuePoller> tmp(new ps::network::QueuePoller(cpu_id(), "SEASTAR2"));
            mQueuePoller2 = std::move(tmp);
        } else {
            //std::cout << "set is_server_client_mode false, not start queue SEASTAR2." << std::endl;
        }
    }
...
// define a signal handler for epoll_pwait signal
void
reactor::epoll_pwait_notifier(int) {
    //std::cout << "call epoll_pwait_notifier ... " << std::endl;
    // Nothing...
}
...
void reactor::configure(boost::program_options::variables_map vm) {
    ...
    _old_max_poll_time = _max_poll_time;
...
//
// poller for server to write packet
//
class reactor::ps_server_write_poller_pollfn final : public reactor::pollfn
{
    reactor& _r;
public:
    ps_server_write_poller_pollfn(reactor& r) : _r(r),
        mWriteQueuePoller(_r.GetWriteQueuePoller())
    {}

    virtual bool poll() final override {
	return mWriteQueuePoller.Available();
    }
...
private:
    ps::network::WriteQueuePoller& mWriteQueuePoller;
};

class reactor::ps_queue_poller_checker_pollfn final : public reactor::pollfn
{
    reactor& _r;
public:
    ps_queue_poller_checker_pollfn(reactor& r) : _r(r),
        mQueuePoller(_r.GetQueuePoller()),
        mQueuePoller2(_r.GetQueuePoller2())
    {}
    virtual bool poll() final override {
	if (smp::seperate_server_client) {
	    // In seperate mode, server cpu_id start from smp::client_count
	    // client cpu_id range [0, smp::client_count)
	    if (_r.cpu_id() >= smp::client_count) {
		return mQueuePoller2.Available();
	    } else {
		return mQueuePoller.Available();
	    }
	}

        if (smp::is_server_client_mode) {
            bool b1 = mQueuePoller.Available();
            bool b2 = mQueuePoller2.Available();
            return b1 || b2;
        }
...
private:
    ps::network::QueuePoller& mQueuePoller;
    ps::network::QueuePoller& mQueuePoller2;
};

// Deprecated!
class reactor::ps_queue_hub_pollfn final : public reactor::pollfn
{
    reactor& _r;
public:
    ps_queue_hub_pollfn(reactor& r,
            std::pair<ps::network::QueueHub<ps::network::Item>*,
            ps::network::QueueHub<ps::network::Item>*> qhr) :
        _r(r), mInputHub(qhr.first), mOutputHub(qhr.second),
        kProducerCount(mInputHub->GetMCount()), kMyCpuId(engine().cpu_id()), mFlag(false),
        mRunning(false),
        mQueuePoller(kMyCpuId)
    {}
    virtual bool poll() final override
    {
        return mQueuePoller.Poll();
    }
    virtual bool pure_poll() override final {
        return poll();
    }
...
int reactor::run() {
    ...
    // server and client will running in different core,
    // so, if in server engine, the mQueuePoller will be NULL.
    //
    if (mQueuePoller) {
        //std::cout << "run mQueuePoller" << std::endl;
        mQueuePoller->Poll();
    }
    // mQueuePoller2 will be started only at server_client mode
    if (mQueuePoller2) {
        //std::cout << "run mQueuePoller2" << std::endl;
        mQueuePoller2->Poll();
    }

    if (mWriteQueuePoller) {
        //std::cout << "===================> start mWriteQueuePoller" << std::endl;
        mWriteQueuePoller->Poll();
    }
    ...
    //    = ps::network::QueueHubFactory::GetInstance().GetHub<ps::network::Item>("SEASTAR", smp::user_threads_num, smp::count);
    //poller ps_queue_hub_poller(std::make_unique<ps_queue_hub_pollfn>(*this, qhr));
    poller ps_queue_poller_checker_poller(std::make_unique<ps_queue_poller_checker_pollfn>(*this));

    //
    // server write packet queue poller
    //
    poller ps_server_write_poller(std::make_unique<ps_server_write_poller_pollfn>(*this));
    ...
    // register a signal for wake up epoll_pwait when data received by write queue
    struct sigaction sa_epoll_pwait_notifier = {};
    sa_epoll_pwait_notifier.sa_handler = &reactor::epoll_pwait_notifier;
    sa_epoll_pwait_notifier.sa_flags = SA_RESTART;
    auto r2 = sigaction(SIGWAKEUPEPOLL, &sa_epoll_pwait_notifier, nullptr);
    assert(r2 == 0);
...
/*
                    ...
                    // qhr.first will notify the thread when call enqueue
                    qhr.first->SetNeedNotifyEpoll(engine().cpu_id(), true);
                    if (qhr.first->GetTargetSeastarThreadId(engine().cpu_id()) == 0) {
                        qhr.first->SetTargetSeastarThreadId(engine().cpu_id(), pthread_self());
                    }
*/

                    unsigned clientQueueId = engine().cpu_id();
		    unsigned serverQueueId = engine().cpu_id();
		    if (smp::seperate_server_client) {
		        serverQueueId -= smp::client_count;
		    }

		    if (mQueuePoller) {
                        mQueuePoller->GetInputHub()->SetNeedNotifyEpoll(clientQueueId, true);
                        if (mQueuePoller->GetInputHub()->GetTargetSeastarThreadId(clientQueueId) == 0) {
                            mQueuePoller->GetInputHub()->SetTargetSeastarThreadId(clientQueueId, pthread_self());
                        }
		    }

		    // In seperate server_client mode, server cpu id start from 0,
		    // so we need not switch cpu id to queue id
		    //
                    // Only in server_client mode
                    if (mQueuePoller2) {
                        mQueuePoller2->GetInputHub()->SetNeedNotifyEpoll(serverQueueId, true);
                        if (mQueuePoller2->GetInputHub()->GetTargetSeastarThreadId(serverQueueId) == 0) {
                            mQueuePoller2->GetInputHub()->SetTargetSeastarThreadId(serverQueueId, pthread_self());
                        }
                    }
...
/*
                    qhr.first->SetNeedNotifyEpoll(engine().cpu_id(), false);
                    // Do NOT set back to 0
                    //qhr.first->SetTargetSeastarThreadId(0);
*/

		    if (mQueuePoller) {
	                mQueuePoller->GetInputHub()->SetNeedNotifyEpoll(clientQueueId, false);
		    }

                    if (mQueuePoller2) {
                        mQueuePoller2->GetInputHub()->SetNeedNotifyEpoll(serverQueueId, false);
                    }
...
// switch mode will be used in normal mode
void
reactor::switch_to_poll_mode() {
    // not the first thread switch to poll-mode
    // and now is in poll-mode already
    if (++_cur_poll_mode_thread_count > 1) {
 	return;
    }
    _max_poll_time = std::chrono::nanoseconds::max();
}
...
void
reactor::switch_to_normal_mode() {
    // there are some others threads in poll-mode
    if (--_cur_poll_mode_thread_count > 0) {
	return;
    }
    _max_poll_time = _old_max_poll_time;
}
...
smp::get_options_description()
{
    ...
    opts.add_options()
        ...
        ("client-cores", bpo::value<unsigned>(), "number of client threads in seperate server_client mode (default: 0)")
        ...
        //("user_threads_num", bpo::value<int64_t>()->default_value(1), "User threads count.")
        //("server_client_mode", bpo::value<bool>()->default_value(false), "wether start server and client at one node")
#ifdef HAVE_HWLOC
...
unsigned smp::client_count = 0;
unsigned smp::server_count = 0;
bool smp::_using_dpdk;
bool smp::is_server_client_mode = false;
bool smp::seperate_server_client = false;
//int64_t smp::user_threads_num = 1;
uint64_t smp::stop_sequence = 0;
int64_t smp::stop_user_thread_id = -1;
...
void smp::configure(boost::program_options::variables_map configuration)
{
    ...
    for (auto sig : {SIGHUP, SIGQUIT, SIGILL, SIGABRT, SIGFPE, SIGSEGV,
            SIGALRM, SIGCONT, SIGSTOP, SIGTSTP, SIGTTIN, SIGTTOU, SIGWAKEUPEPOLL}) {
        sigdelset(&sigs, sig);
    }
    ...
    // If you set this var, that indicate to use sepreate server_client mode
    if (configuration.count("client-cores")) {
        // in server_client mode, if you want to sepreate client and server to different cores,
        // that you must set "client-cores".
        // client will use cores from 1~client-cores-1, core-id: [0, client-cores)
        // server cpu id range: [client-cores, smp::count)
        // ATTENTION: smp::count = smp::server_count + smp::client_count
        //
        smp::seperate_server_client = true;
        smp::client_count = configuration["client-cores"].as<unsigned>();
        if (smp::client_count >= smp::count) {
            print("In seperate server_client mode, client cores must less then smp::count ! ");
            ::exit(-1);
        }
        smp::server_count = smp::count - smp::client_count;
    }

    // user thread count, default=1
    // it will be used for create ps queue
    //if (configuration.count("user_threads_num")) {
    //    smp::user_threads_num = configuration["user_threads_num"].as<int64_t>();
    //} else {
    //    smp::user_threads_num = 1;
    //}

    // server_client_mode: wether start server and client at one node
    if (configuration.count("server_client_mode")) {
        //std::cout << "set server_client mode." << std::endl;
        smp::is_server_client_mode = configuration["server_client_mode"].as<bool>();
    } else {
        //std::cout << "set no server_client mode" << std::endl;
        smp::is_server_client_mode = false;
    }
...
network_stack_registrator nsr_posix{"posix",
	...
	// In seperate server_client mode,
	// we should create posix_network_stack for the first server core thread,
	// you know, in the mode,
	// client cores range: [0, smp::client_count-1),
	// server cores range: [smp::client_count, smp::count)
	// so we should create posix_network_stack for core smp::client_count
	if (smp::seperate_server_client) {
	    // the first server core
	    if (engine().cpu_id() == smp::client_count) {
		return posix_network_stack::create(ops);
	    }
	    return posix_ap_network_stack::create(ops);
	}
        // In normal mode
