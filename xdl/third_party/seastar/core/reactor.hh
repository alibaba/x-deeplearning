...
#include "service/write_queue_poller.hh"
...
public:
    ...
    void close() { _s.reset(); }
    void inc_truely_send_count(int i) { _truely_send_count += i; }
    void reset_truely_send_count() { _truely_send_count = 0; }
    uint64_t get_truely_send_count() const { return _truely_send_count; }
protected:
...
private:
    ...
    // count of call sys send function
    uint64_t _truely_send_count;
};
...
private:
    ...
    class ps_queue_poller_checker_pollfn;
    class ps_server_write_poller_pollfn;
    class ps_queue_hub_pollfn;
...
private:
    ...
    std::chrono::nanoseconds _old_max_poll_time;
...
public:
    // poll-mode: will not sleep
    // normal_mode: will sleep when no work to do
...
private:
    ...
    static void epoll_pwait_notifier(int);
    void wakeup();
    ...
    // save the count of threads which call switch to poll-mode
    unsigned _cur_poll_mode_thread_count;
...
public:
    ...
    ps::network::QueuePoller& GetQueuePoller() const {
        return *mQueuePoller;
    }

    ps::network::QueuePoller& GetQueuePoller2() const {
        return *mQueuePoller2;
    }

    ps::network::WriteQueuePoller& GetWriteQueuePoller() const {
        return *mWriteQueuePoller;
    }
private:
    std::unique_ptr<ps::network::QueuePoller> mQueuePoller;
    std::unique_ptr<ps::network::QueuePoller> mQueuePoller2;

    std::unique_ptr<ps::network::WriteQueuePoller> mWriteQueuePoller;
};
...
public:
    // the below two counts only be used in the server_client mode
    // what's server_client mode ?
    // server and client will running in one process.
    // Which means the node will be acted as server and client in the same time.
    // In server_client mode, count = client_count + server_count
    // client's cpu id: 0 ~ client_count-1
    // server's cpu id: client_count ~ count - 1
    static unsigned client_count;
    static unsigned server_count;

    static unsigned count;
    //static int64_t user_threads_num;
    static uint64_t stop_sequence;
    static int64_t stop_user_thread_id;
    static bool is_server_client_mode;
    static bool seperate_server_client;
};
...
future<size_t> pollable_fd::write_some(net::packet& p) {
    return engine().writeable(*_s).then([this, &p] () mutable {
        ...
        msghdr mh = {};
        mh.msg_iov = iov;
        mh.msg_iovlen = std::min(p.nr_frags(), (unsigned)UIO_MAXIOV);
        //{
            //for (int32_t i = 0; i < mh.msg_iovlen; i += 3)
            //{
                //iovec* curIoVec = mh.msg_iov + i;
                //char* ptr = reinterpret_cast<char*>(curIoVec->iov_base) + curIoVec->iov_len - 8;
                //*reinterpret_cast<uint64_t*>(ptr)
                    //= CoutCurrentTimeOnNanoSec("__________pollable_fd__write_some", false);
            //}
        //}
        auto r = get_file_desc().sendmsg(&mh, MSG_NOSIGNAL);
        this->inc_truely_send_count(1);
        if (!r) {
            return write_some(p);
        }
        if (size_t(*r) == p.len()) {
