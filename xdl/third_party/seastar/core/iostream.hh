...
public:
    ...
    // get data and put data into the memory that user offered
    // TODO: posix stack and native stack
    virtual future<size_t> get(char* user_buf, size_t n) {
        // not impl, false always
        assert(false);
...
public:
    ...
    // new try: read into memory that user offered
    future<size_t> get(char* user_buf, size_t n) { return _dsi->get(user_buf, n); }
};
...
public:
    ...
    virtual uint64_t get_truely_send_count() { return 0; }
    virtual void reset_truely_send_count() {}
};
...
public:
    ...
    future<> close() { return _dsi->close(); }
    uint64_t get_truely_send_count() { return _dsi->get_truely_send_count(); }
    void reset_truely_send_count() { return _dsi->reset_truely_send_count(); }
};
...
public:
    ...
    // read data into the memory that user offered
    future<size_t> read_exactly_into(size_t n, CharType* user_buf);
...
class output_stream final {
    ...
    // the lock will be use by zero_copy_put and poll_flush
    rwlock _lock;
private:
...
public:
	...
	: _fd(std::move(fd)), _size(size), _trim_to_size(trim_to_size), _batch_flushes(batch_flushes)
        //: _fd(std::move(fd)), _size(size), _trim_to_size(true), _batch_flushes(batch_flushes)
    {
        //std::cout << "output_stream,size:" << size
    ...
    /// set _batch_flushes by yourself
    void set_batch_flushes(bool batch_flush);

    /// get the truely sys send call count
    uint64_t get_truely_send_count();

    /// set count = 0
    void reset_truely_send_count();
private:
