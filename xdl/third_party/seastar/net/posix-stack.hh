...
class conntrack {
    ...
    public:
        ...
        /*      
        // deprecated: not elegant
        load_balancer() : _cpu_load(size_t(smp::count), 0) {
            if (smp::seperate_server_client) {
                // in seperate mode, server cpu id is start from 'smp::client_count'
                // client id range [0, smp::client_count)
                // server id range [smp::client_count, smp::count)
                // _next_server_cpu_id will be used when client core recv a accept request,
                // it will submit the request to server core: _next_server_cpu_id,
                // then _next_server_cpu_id++;
    	        _next_server_cpu_id = smp::client_count;
            }
        }
        */

	load_balancer() : _cpu_load(smp::seperate_server_client 
	    ? size_t(smp::server_count) : size_t(smp::count), 0) {}
        ...
        /* 
        // deprecated: not elegant
        // only used in seperate server_client mode
        // for load balance
        // please see the usage in @posix_server_socket_impl<Transport>::accept()
        unsigned _next_server_cpu_id;
        */
    };
...
// deprecated: not elegant
public:
    unsigned get_next_server_cpu_id() {
        return _lb->_next_server_cpu_id;
    }
    void set_next_server_cpu_id(unsigned id) {
        _lb->_next_server_cpu_id = id;
    }
*/
public:
    class handle {
...
public:
    ...
    future<> close() override;
    uint64_t get_truely_send_count() override;
    void reset_truely_send_count() override;
};
...
public:
    ...
    virtual void abort_accept() override;
//    conntrack& get_conntrack() { return _conntrack; }
};
