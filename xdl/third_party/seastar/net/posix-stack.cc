...
/*      // deprecated: not elegant
	// In seperate mode, all cores include server cores and client cores
	// must call accept function(seastar bug).
	// so if client cores recv the accept request, we should submit it to
	// server cores
	if (smp::seperate_server_client) {
	    // client core recv a accept request
	    if (cpu < smp::client_count) {
    	        cpu = this->get_conntrack().get_next_server_cpu_id();
    	        if (cpu + 1 >= smp::count) {
		    this->get_conntrack().set_next_server_cpu_id(smp::client_count);
    	        } else {
		    this->get_conntrack().set_next_server_cpu_id(cpu + 1);
   	        }
	    }
	}
*/
        ...
        if (smp::seperate_server_client) {
            cpu += smp::client_count;
        }

        //std::cout << "exec accept cpu id = " << cpu << ", current cpu = " << engine().cpu_id() << std::endl;
...
uint64_t
posix_data_sink_impl::get_truely_send_count() {
    return _fd->get_truely_send_count();
}

void
posix_data_sink_impl::reset_truely_send_count() {
