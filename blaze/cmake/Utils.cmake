...
# Usage:
#   blaze_list_unique(<list_variable> [<list_variable>] [...])
macro(blaze_list_unique)
  foreach(__lst ${ARGN})
    if(${__lst})
...
# Usage:
#   blaze_clear_vars(<variables_list>)
macro(blaze_clear_vars)
  foreach(_var ${ARGN})
    unset(${_var})
  endforeach()
endmacro()
...
# Usage:
#   blaze_string_unique(<string_variable>)
function(blaze_string_unique __string)
  if(${__string})
    set(__list ${${__string}})
...
# Usage:
#   blaze_print_list(<list>)
function(blaze_print_list)
  foreach(e ${ARGN})
    message(STATUS ${e})
  endforeach()
endfunction()
...
# Usage:
#   blaze_merge_flag_lists(out_variable <list1> [<list2>] [<list3>] ...)
function(blaze_merge_flag_lists out_var)
  set(__result "")
  foreach(__list ${ARGN})
...
# Usage:
#   blaze_convert_absolute_paths(<list_variable>)
function(blaze_convert_absolute_paths variable)
  set(__dlist "")
...
# Usage:
#   blaze_parse_header(<file> <define1> <define2> <define3> ..)
macro(blaze_parse_header FILENAME FILE_VAR)
  set(vars_regex "")
...
# Usage:
#   blaze_parse_header_single_define(<library_name> <file> <define_name>)
function(blaze_parse_header_single_define LIBNAME HDR_PATH VARNAME)
  set(${LIBNAME}_H "")
  if(EXISTS "${HDR_PATH}")
...
# Usage:
#   blaze_option(<option_variable> "doc string" <initial value or boolean expression> [IF <condition>])
function(blaze_option variable description value)
  set(__value ${value})
  set(__condition "")
...
# Usage:
#   blaze_compare_lists(<list_variable> <list2_variable> [description])
function(blaze_compare_lists list1 list2 desc)
  set(__list1 ${${list1}})
  set(__list2 ${${list2}})
  list(SORT __list1)
  list(SORT __list2)
...
# Usage:
#   blaze_warnings_disable(<CMAKE_[C|CXX]_FLAGS[_CONFIGURATION]> -Wshadow /wd4996 ..,)
macro(blaze_warnings_disable)
  set(_flag_vars "")
  set(_msvc_warnings "")
  set(_gxx_warnings "")
  ...
  blaze_clear_vars(_flag_vars _msvc_warnings _gxx_warnings)
endmacro()

################################################################################################
# Helper function get current definitions
# Usage:
#   blaze_get_current_definitions(<definitions_variable>)
function(blaze_get_current_definitions definitions_var)
...
# Usage:
#   blaze_get_current_cflags(<cflagslist_variable>)
function(blaze_get_current_cflags cflags_var)
  get_property(current_includes DIRECTORY PROPERTY INCLUDE_DIRECTORIES)
  blaze_convert_absolute_paths(current_includes)
  blaze_get_current_definitions(cflags)

  foreach(i ${current_includes})
    list(APPEND cflags "-I${i}")
  endforeach()

  blaze_list_unique(cflags)
...
# Usage:
#   blaze_parse_linker_libs(<blaze_LINKER_LIBS_var> <directories_var> <libflags_var> <frameworks_var>)
function(blaze_parse_linker_libs blaze_LINKER_LIBS_variable folders_var flags_var frameworks_var)
  ...
  foreach(list_elem ${${blaze_LINKER_LIBS_variable}})
  ...
  blaze_list_unique(libflags folders)
...
# Usage:
#   blaze_detect_darwin_version(<version_variable>)
function(blaze_detect_darwin_version output_var)
  if(APPLE)
...
function(blaze_source_group group)
...
function(blaze_add_test dir extension)
  ...
  foreach(file ${TEST_SOURCE})
    ...
    set_target_properties(${file_exe} PROPERTIES COMPILE_FLAGS "-g --coverage -Dprivate=public -Dprotected=public")
    add_test(${file_exe} ${file_exe} COMMAND ${file_exe})
  endforeach()
endfunction()

function(blaze_add_cuda_test dir extension)
  file(GLOB TEST_SOURCE "${dir}/*${extension}")
  foreach(file ${TEST_SOURCE})
    string(REGEX REPLACE ".*${dir}/" "" file_exe0 ${file})
    string(REPLACE "${extension}" "" file_exe ${file_exe0})
...
function(blaze_add_bench dir extension)
  file(GLOB TEST_SOURCE "${dir}/*${extension}")
  foreach(file ${TEST_SOURCE})
    string(REGEX REPLACE ".*${dir}/" "" file_exe0 ${file})
    string(REPLACE "${extension}" "" file_exe ${file_exe0})
...
function(blaze_add_cuda_bench dir extension)
  file(GLOB TEST_SOURCE "${dir}/*${extension}")
  foreach(file ${TEST_SOURCE})
    string(REGEX REPLACE ".*${dir}/" "" file_exe0 ${file})
    string(REPLACE "${extension}" "" file_exe ${file_exe0})
