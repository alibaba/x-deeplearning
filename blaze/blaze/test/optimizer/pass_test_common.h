/*
 * \file pass_test_common.h
 * \brief The pass test common
 */
...
void CheckPassOutput(const char* net_conf_path, const char* expected_net_conf_path) {
  Workspace workspace;
  std::shared_ptr<Net> net = workspace.CreateNet(net_conf_path);
  NetDef net_def = net->net_def();

  // Start to pass
  net_def = Optimizer::Get()->RunPass(net_def);
...
void CheckPassOutputWithoutLoad(const char* net_conf_path, const char* expected_net_conf_path) {
  NetDef net_def, expected_net_def;
  EXPECT_TRUE(NetDefHelper::LoadNetDefFromTextFile(net_conf_path, &net_def));
  EXPECT_TRUE(NetDefHelper::LoadNetDefFromTextFile(expected_net_conf_path, &expected_net_def));
  // Start to pass
  net_def = Optimizer::Get()->RunPass(net_def);
  LOG_INFO("FusionPatternPass Result: %s", net_def.DebugString().c_str());
