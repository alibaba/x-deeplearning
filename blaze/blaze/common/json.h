 ...
 *  NOTE: The code is from MXNet for MXNet model parse
 */
#pragma once
...
class JSONReader {
 public:
   ...
   * \throw blaze::Error when next token is not string
   */
  inline void ReadString(std::string *out_str);
  /*!
   * \brief Read Number.
   * \param out_value output value;
   * \throw blaze::Error when next token is not number of ValueType.
   ...
   * \throw blaze::Error when the read of ValueType is not successful.
 ...
 *    inline void Load(blaze::JSONReader *reader) {
 *      blaze::JSONStructReadHelper helper;
...
#define BLAZE_JSON_ENABLE_ANY_VAR_DEF(KeyName)                  \
  static ::blaze::json::AnyJSONManager&   \
  __make_AnyJSONType ## _ ## KeyName ## __

/*!
 * \def BLAZE_JSON_ENABLE_ANY
 * \brief Macro to enable save/load JSON of blaze:: whose actual type is Type.
 ...
 */
#define BLAZE_JSON_ENABLE_ANY(Type, KeyName)                             \
  CONCATENATE(BLAZE_JSON_ENABLE_ANY_VAR_DEF(KeyName), __COUNTER__) = \
    ::blaze::json::AnyJSONManager::Global()->EnableType<Type>(#KeyName) \
...
struct ArrayHandler {
  inline static void Write(JSONWriter *writer, const ContainerType &array) {
    ...
    writer->BeginArray(array.size() > 10 || !blaze::is_pod<ElemType>::value);
...
struct Handler<std::pair<K, V> > {
  ...
  inline static void Read(JSONReader *reader, std::pair<K, V> *kv) {
    reader->BeginArray();
    CHECK_TRUE(reader->NextArrayItem(), "Expect array of length 2");
...
struct Handler {
  inline static void Write(JSONWriter *writer, const T &data) {
    typedef typename blaze::IfThenElseType<blaze::is_arithmetic<T>::value,
                                          NumericHandler<T>,
 ...
 public:
  ...
  inline AnyJSONManager& EnableType(const std::string& type_name) {  // NOLINT(*)
    ...
    if (type_name_.count(tp) != 0) {
      CHECK_TRUE(type_name_.at(tp) == type_name,
          "Type has already been registered as another typename " , type_name_.at(tp));
      return *this;
    }
    CHECK_TRUE(type_map_.count(type_name) == 0,
...
struct Handler<any> {
  inline static void Write(JSONWriter *writer, const any &data) {
    ...
    CHECK_TRUE(it != nmap.end() && it->first == id,
  ...
  inline static void Read(JSONReader *reader, any *data) {
    ...
    reader->BeginArray();
    CHECK_TRUE(reader->NextArrayItem(), "invalid any json format");
    ...
    CHECK_TRUE(it != tmap.end() && it->first == type_name,
  ...
  while (true) {
    ...
    if (ch == '\\') {
      ...
      switch (sch) {
        ...
        case '\"': Extend(&output, "\""); break;
        default: BLAZE_THROW("Unkown string escape \\", sch);
      }
    } else {
      if (ch == '\"') break;
    ...
    if (ch == EOF || ch == '\r' || ch == '\n') {
      BLAZE_THROW("Error at ", line_info(), ", Expect \'\"\' but reach end of line ");
    }
  }
#ifndef _LIBCPP_SGX_NO_IOSTREAMS
  *out_str = output.str();
...
#ifndef _LIBCPP_SGX_NO_IOSTREAMS
  *is_ >> *out_value;
  CHECK_TRUE(!is_->fail(), "Error at: ", line_info(), ", Expect number");
#else
  char* endptr;
...
inline void JSONReader::BeginObject() {
  int ch = NextNonSpace();
  CHECK_EQ(ch, '{', "Error at", line_info(),
           ", Expect \'{\' but get \'", static_cast<char>(ch), '\'');
